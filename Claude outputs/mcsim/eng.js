...
function startDay(){
  for(const p of G.players){p.influence=copies(p,'Esperienza');p.subsLeft=copies(p,'Contrattazione Sottobanco');p.fortunaLeft=copies(p,'Nuovi Clienti');p.passedAll=false;p.today={contracts:[],income:0,bilancia:[]}}
...
function finishDraft(){
  ...
  G.phase='asta';G.auctionIndex=-1;nextAuction();
}

// ---------------- Fase 2 · Aste ----------------
function currentFish(){return FISH[G.auctionIndex]}
...
window.submitBid=function(mode='bid'){
  ...
  for(const p of G.players)if(!p.human&&!(p.id===0&&mode==='bid')){const b=botBid(p,f);if(b)bids.push(b)}
...
function botUpgradeValue(p,c){
  ...
  if(c.up==='Esperienza')v+=daysLeft*1.1*ESP_X;
...
function feasible(p,c){const stock=mix(p.banco,p.cesta);return Object.entries(c.recipe).every(([g,n])=>{const miss=n-(stock[g]||0);if(miss<=0)return true;const gi=FISH.indexOf(g);return gi>=G.auctionIndex&&(G.market[g]||0)>=miss})}
...
function botBid(p,f){
  ...
  let card=bidCards.find(c=>c.bid+p.influence*ESP_X>=target)||bidCards[0];
  if(!card){if(need<2)return null;card=[...p.hand].sort((a,b)=>botCardValue(p,a)-botCardValue(p,b))[0]}
  const infl=Math.min(p.influence,Math.ceil(Math.max(0,target-card.bid)/ESP_X));
  // Vincere l'asta fa risparmiare ~1 Ducato per pesce: il bot non punta più Ducati di quanti ne risparmia.
  const cap=need;
  const cash=Math.max(0,Math.min(p.coins-need*2,cap,target-card.bid-infl*ESP_X));
...
botBid=function(p,f){
  ...
  if(G.inRollout||!MC.on||!MC.bid||!p.mc||!p.hand.length)return _botBid(p,f);
  const b0=_botBid(p,f),opts=[null],key=new Set(['x']);
